(function ($) {
  ...
  Backdrop.behaviors.halloweenLights = {
    attach: function () {
      ...
      // ---------------- helpers ----------------
      function settings() {
        return Backdrop.settings.halloween_lights || {};
      }
      ...
      function setLightsBackgroundFromHeader($header, $lights) {
        try {
          var bg = getComputedStyle($header[0]).backgroundColor;
          if (bg) $lights.css('backgroundColor', bg);
        } catch (e) {}
      }

      // Height of admin UI sitting above the page.
      function getAdminOffset() {
        var s = settings();
        var px = 0;

        // Backdrop admin_menu bar.
        if (s.lightsAdminMenu && $('#admin-bar:visible').length) {
          px = Math.max(px, $('#admin-bar').outerHeight(true) || 0);
        }
        // Backdrop core toolbar.
        if (s.lightsCoreToolbar && $('#toolbar:visible').length) {
          px = Math.max(px, $('#toolbar').outerHeight(true) || 0);
        }
        // Optional Bootstrap fixed navbar.
        if (s.lightsBootstrapFixed && $('.navbar-fixed-top').length) {
          var $nb = $('.navbar-fixed-top').first();
          px = Math.max(px, (($nb.position().top || 0) + ($nb.outerHeight(true) || 0)));
        }
        return px;
      }
      ...
      function setPumpkins() {
        ...
        // Clear any prior blink intervals.
        var ids = $wrapper.data('hlIntervals') || [];
        ...
        if (created) {
          ...
          $wrapper.data('hlIntervals', ids);
        }
      }

      // ---------------- behaviors ----------------
      // Non-fixed: lives inside header; hide on scroll.
      ...
      // Push header and position fixed lights under admin UI.
      function applyHeaderOffset() {
        ...
        if (!fixed) {
          // Non-fixed: custom spacing inside the bar; header at base.
          $lights.css({ top: '', paddingTop: hlCustomTop() + 'px' });
          $header.css('padding-top', basePadding + 'px');
          setLightsBackgroundFromHeader($header, $lights);
          return;
        }

        // Fixed: place lights below admin UI and push header by lights+admin.
        $lights.css('top', adminOffset + 'px');

        var H = $lights.outerHeight(true) || 0; // includes custom padding
        $header.css('padding-top', (basePadding + H + adminOffset) + 'px');
      ...
      // Some admin bars finish sizing after load.
