    ...
    attach: function (context, settings) {
      ...
      function applyHeaderOffset() {
        ...
        if (!fixed) {
          ...
          $lights.css('padding-top', hlCustomTop() + 'px');  // paints the right bg
          $header.css('padding-top', basePadding + 'px');    // header stays at base
          return;
        }

        // FIXED: the bar already has padding-top (custom). Push header by bar height.
        var H = $lights.outerHeight(true) || 0; // includes that padding
        $header.css('padding-top', (basePadding + H) + 'px');

        // Keep the fixed bar visually matching the header (safety).
